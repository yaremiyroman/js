...
const records = process.env.AMOUNT || 10;
const fileName = process.env.FILE_NAME || 'myDB.json';
...
function generateDB() {
    const items = [];
    const myFilePath = path.resolve(appRoot, fileName);

    for (let i = 1; i <= records; i++) {
        items.push({
            id: i,
            name: getRandomLorem(1),
            body: getRandomLorem(10),
        });
    }
